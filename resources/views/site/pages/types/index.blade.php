    ...
    <section class="main_section page_head">
        <div class="container">
            <div class="row text-center">
                <div class="col-12">
                    <h3>{{ $type->translate(app()->getLocale())->name }}</h3>
                    <ul>
                        <li><a href="{{ route('site.index') }}">
                                {{ app()->getLocale() == 'en' ? 'Home' : 'الرئيسية' }}</a></li>
                        <li>{{ $type->translate(app()->getLocale())->name }}</li>
                    </ul>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                ...
                <div class="col-12">
                    <div class="owl-carousel owl-theme">
                        @foreach ($type->projects as $featured_project)
                            @if ($featured_project->type == 1)
                                <div class="item">
                                    <a href="{{ route('site.project', ['project' => $featured_project->slug]) }}"
                                        ...
                                        @if ($featured_project->status->translate(app()->getLocale())->name)
                                            <span class="tag">
                                                {{ $featured_project->translate(app()->getLocale())->status }}
                                            </span>
                                        @endif
